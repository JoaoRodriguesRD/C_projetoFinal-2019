...
void bolao_cadastrar(tbolao bolao, tapostadores apost[MAX_APOSTADORES])
{
	...
	if( bolao.data.dia != dia && bolao.data.mes != mes && bolao.data.ano != ano ) //Data diferente da armazenada!
	{
		do
		{
		...
		bolao.bolaoU.quant = 0;
		bolao.bolaoV.quant = 0;
		bolao.bolaoU.finalizado = 0;
		bolao.bolaoV.finalizado = 0;
		...
		case 'u':
			...
			for(i = 0; i < n; i++)
			{
				...
				{
					if(indice == bolao.bolaoU.apostador[j])
					{
				...
				registrado++;
				bolao.bolaoU.apostador[bolao.bolaoU.quant] = indice;
			...
			bolao.bolaoU.quant = bolao.bolaoU.quant + registrado;

			printf("Digite a quantidade de numeros no cartao\n");
			do
			{
				printf("No minimo %d e no maximo %d: ", MIN_NUMEROS, MAX_NUMEROS);
				scanf("%d", &bolao.bolaoU.quant_numeros);
			}
			while(bolao.bolaoU.quant_numeros < MIN_NUMEROS || bolao.bolaoU.quant_numeros > MAX_NUMEROS);

			do
			{
				printf("Digite %d numeros:", bolao.bolaoU.quant_numeros);
				for(i = 0; i < bolao.bolaoU.quant_numeros; i++)
					scanf("%d", &bolao.bolaoU.numeros[i]);
				if(numero_repetido(bolao.bolaoU.numeros, bolao.bolaoU.quant_numeros))
					printf("Nao pode ter sorteio com numero repetido!\n");
			}
			while(numero_repetido(bolao.bolaoU.numeros, bolao.bolaoU.quant_numeros));
		...
		case 'v':
			...
			for(i = 0; i < n; i++)
			{
				...
				for(j = 0; j < MAX_APOSTADORES; j++)
				{
					if(indice == bolao.bolaoV.apostador[j])
					{
						printf("Apostador inserido anteriormente\n");
						continua = 1;
					}
				...
				printf("registrado\n");
				registrado++;
				bolao.bolaoV.apostador[bolao.bolaoV.quant] = indice;

			}
			bolao.bolaoV.quant += registrado;
			printf("Foram registrados %d apostadores para esse bolao\n", registrado);
			if(registrado == 0)
			{
				printf("Retornando ao menu...\n");
			...
			for(i = 0; i < cartoes; i++)
			{
				...
				do
				{
					...
					for(k = 0; k < quantidade; k++)
						scanf("%d", &bolao.bolaoV.numeros[i][k]);
					if(numero_repetido(bolao.bolaoV.numeros[i], quantidade))
						printf("Nao pode ter sorteio com numero repetido!\n");
				}
				while(numero_repetido(bolao.bolaoV.numeros[i], quantidade));
	...
	{
		if(bolao.bolaoU.quant != 0 && bolao.bolaoV.quant != 0)
		{
...
//altera um bolao
//Recebe o bolao e o vetor de apostadores
void bolao_alterar(tbolao bolao, tapostadores apost[MAX_APOSTADORES])
{
	char opcao;
	int vaga;
	int n;
	...
	case 'u':
		...
		break;
	case 'v':
		if(bolao.bolaoV.finalizado == 1)
		{
			printf("Bolao finalizado, nao pode alterar\n");
			return;
		}
		if(bolao.bolaoV.quant == 0)
		{
			printf("Nao tem apostadores para alterar, por favor cadastre\n");
			return;
		}
...
int menu_apostas(tapostadores apost[MAX_APOSTADORES], int *contador_apost, tbolao bolao)
{
	int opcao;
	/*
	1- Novo
...
int main()
{
	...
	for(i = 0; i < MAX_APOSTADORES; i++)
	{
		bolao.bolaoU.apostador[i] = '\n';
		bolao.bolaoV.apostador[i] = '\n';
	}
	do
	{
